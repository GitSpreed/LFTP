	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				if (packet.isSYN()) {
					if (packet.isREQ()) {
						LFTPGet temp = new LFTPGet(p.getAddress(), port++, packet.getSrcPort(), UDPGetPort, listLock, list, socketLock, socket);
						temp.replyHello();
						threadPool.add(temp);
						temp.start();
					} else {
						LFTPSend temp = new LFTPSend(p.getAddress(), port++, packet.getSrcPort(), UDPGetPort, listLock, list, socketLock, socket);
						temp.replyHello();
						threadPool.add(temp);
						temp.start();
					}
